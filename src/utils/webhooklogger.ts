...
export class WebhookLogger {
	...
	static initialize() {
		...
		if (webhookUrl) {
			try {
				WebhookLogger.webhookClient = new WebhookClient({ url: webhookUrl });
				// Use original info logger so we don't trigger forwarding here
				try {
					WebhookLogger.originalLoggerInfo(
						"Webhook logger initialized successfully",
					);
				} catch {}
			...
			try {
				WebhookLogger.originalLoggerWarn(
	...
	static async logWarning(
		...
		if (!WebhookLogger.webhookClient) return;
		...
		try {
			...
			await WebhookLogger.webhookClient.send({
				embeds: [embed],
